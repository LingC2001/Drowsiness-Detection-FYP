...
fs = 60
folder_path = "DATA/BULK_PKL_BZ2"
with open("M_A_N_vs_blinking_rate.csv", 'w', newline='') as out_f:
    writer = csv.writer(out_f)
    writer.writerow(["Morning", "Afternoon", "Night"])
    files_to_analyse = []
    for f in os.listdir(folder_path):
        if f.endswith("morning_features.pkl") or f.endswith("afternoon_features.pkl") or f.endswith("night_features.pkl"):   # change this later
    ...
    while files_to_analyse != []:
        ...
        data = pd.read_pickle(folder_path + '/' + curr_file, compression="bz2")
        ...
        if curr_file.endswith("morning_features.pkl"):
            curr_row_to_write[0] = avg_blink_rate_per_min
        elif curr_file.endswith("afternoon_features.pkl"):
            curr_row_to_write[1] = avg_blink_rate_per_min
        elif curr_file.endswith("night_features.pkl"):
